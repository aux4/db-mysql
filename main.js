...
function validateArgs() {
  ...
  if (args.length < 5) {
    console.error("Usage: aux4-db-mysql <host> <port> <database> <user> <password>");
    process.exit(1);
  }
  return {
    host: args[0],
...
function convertParameterSyntax(sql) {
  // mysql2 with namedPlaceholders supports :paramName natively
  return sql;
}
...
async function executeQuery(connection, request) {
  ...
  try {
    const [rows] = await connection.execute(convertedSql, sqlParams);
...
async function processItemInBatch(connection, convertedSql, originalSql, item, request, errors) {
  try {
    const cleanItem = filterAux4Params(item);
    const [rows] = await connection.execute(convertedSql, cleanItem);
    if (Array.isArray(rows)) {
...
async function executeBatchWithTransaction(connection, request) {
  ...
  try {
    ...
    for (const item of request.items) {
      const result = await processItemInBatch(connection, convertedSql, request.sql, item, request, errors);
      if (result.success) {
...
async function processStreamItem(connection, convertedSql, originalSql, item, request) {
  try {
    const cleanItem = filterAux4Params(item);
    const [rows] = await connection.execute(convertedSql, cleanItem);
    if (Array.isArray(rows)) {
...
async function streamBatch(connection, request) {
  ...
  try {
    if (request.tx) {
      ...
      for (const item of request.items) {
        const result = await processStreamItem(connection, convertedSql, request.sql, item, request);
      ...
      for (const item of request.items) {
        await processStreamItem(connection, convertedSql, request.sql, item, request);
      }
    }
  } catch (error) {
    try {
